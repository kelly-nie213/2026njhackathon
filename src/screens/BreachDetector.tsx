...
function ReportView(props: {
  orgName: string; crawl: CrawlResult; lookup: BreachLookup;
  report: BreachReport; reportSource: "ai" | "fallback"; onReset: () => void;
}) {
  ...
  return (
    ...
    >
      ...
      {lookup.source === "demo" && crawl.emails.length > 0 && (
        <div className="rounded-xl border border-risk-med/30 bg-risk-med/10 px-3 py-2.5 text-xs text-risk-med">
          ⚠ Couldn't reach the breach database — results are{" "}
          <span className="font-semibold">simulated</span>. Rescan for live data.
        </div>
      )}
